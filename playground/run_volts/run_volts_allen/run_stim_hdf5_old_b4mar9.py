...
def run_model(param_set, sweep, ntimestep):
    ...
    utils.setup_iclamp2(stimulus_path, sweep=int(sweep[:2]), stim=stims_hdf5[sweep], dt=stims_hdf5[sweep + "_dt"][:][0])
    ...
    timescale = int(len(res['v']) / ntimestep)
    res['v'] = res['v'][::timescale]
    res['v'] = res['v'][:ntimestep]
    res['v'] = res['v']*1000#[:20000]
