...
def log_params(maxs, mins, orig_params):
    ...
    new_mins, new_maxs, new_orig_params = [], [], []
    for i in range(len(bases)):
        if bases[i] < config.base_thresh:
            new_orig_params.append(orig_params[i])
            new_mins.append(mins[i])
            new_maxs.append(maxs[i])
        else:
            new_orig_params.append(math.log(orig_params[i],bases[i]))
            new_mins.append(math.log(mins[i],bases[i]))
            new_maxs.append(math.log(maxs[i],bases[i]))

    new_orig_params = np.array(new_orig_params)
    new_mins = np.array(new_mins)
    new_maxs = np.array(new_maxs)
    return bases, new_orig_params, new_mins, new_maxs
...
def evaluate_score_function(stim_names, target_volts, data_volts, weights, dt=None):
    ...
    if psv_scores and not config.passive_scaler:
        config.passive_scaler = actv_scores / psv_scores * config.PASSIVE_PERCTENAGE
    else:
        print("WARNING: no passive stims selected")
    total_score = psv_scores * config.passive_scaler + actv_scores
    print('ACTIVE :', actv_scores, "PASV:", psv_scores * config.passive_scaler)
    return total_score
