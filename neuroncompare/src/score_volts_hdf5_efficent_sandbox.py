...
import neuroncompare.src.score_normalizer as sn
import efel
import h5py
import math
...
for k in range(len(volts_name_list)):
    ...
    if COMM.rank == 0:
        ...
        for i in range(len(score_functions)):
            ...
            sampled_pin_scores = np.array([pin_scores[p_ind] for p_ind in params_sample_pin_ind if p_ind < len(pin_scores) ])
